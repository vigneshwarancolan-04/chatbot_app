...
def load_history(session_id):
    try:
        db = get_db()
        cursor = db.cursor()
        cursor.execute("SELECT question, answer FROM chat_history WHERE session_id = %s ORDER BY timestamp", (session_id,))
...
def save_chat(session_id, question, answer):
    try:
        ...
        if count == 0:
            ...
            cursor.execute("INSERT INTO sessions (id, title, created_at) VALUES (%s, %s, %s)",
                           (session_id, title, datetime.datetime.now()))

        cursor.execute("INSERT INTO chat_history (session_id, question, answer, timestamp) VALUES (%s, %s, %s, %s)",
